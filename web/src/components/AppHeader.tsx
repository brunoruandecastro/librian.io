...
import { Search, User, BookOpen, Heart, Settings, Menu, X, Bell, Plus, Library, BookMarked, LogOut, LogIn } from "lucide-react"
    ...
    <header className="w-full px-4 sm:px-6 py-4 bg-surface/95 backdrop-blur-xl sticky top-0 z-50 shadow-sm relative">
      ...
      <div className="max-w-7xl mx-auto flex items-center justify-between h-16">
        ...
        <div className="flex items-center gap-6">
          ...
          {/* Desktop Navigation - só mostra se logado */}
          {user && (
          ...
          {user ? (
            ...
            <>
              ...
              <div className="flex items-center gap-3">
                ...
                >
                  <div className="w-10 h-10 rounded-2xl border border-accent/20 group-hover:border-accent/40 transition-all duration-200 overflow-hidden relative">
                    <img 
                    ...
                    />
                    {imageError && (
                      <div className="absolute inset-0 bg-gradient-to-br from-accent-light/20 via-accent/20 to-accent-dark/20 flex items-center justify-center">
                  ...
                  <div className="hidden sm:block text-left">
                    ...
                    <p className="text-xs text-text-secondary">Usuário</p>
                  </div>
                </Link>
                <button
                  onClick={handleLogout}
                  className="w-12 h-12 text-text-secondary hover:text-rose-500 transition-all duration-200 hover:bg-rose-50 dark:hover:bg-rose-900/20 rounded-2xl hover:scale-105 flex items-center justify-center"
                  title="Sair"
                >
            ...
            <Link
              href="/login"
              className="flex items-center gap-3 px-6 py-3 bg-gradient-to-r from-accent to-accent-dark text-white rounded-2xl hover:shadow-lg transition-all duration-200 hover:scale-105 font-semibold"
            >
          ...
          <div className="pt-6 px-4 space-y-3">
            {user ? (
              ...
              <>
                ...
                <button
                  onClick={handleLogout}
                  className="w-full flex items-center gap-4 px-4 py-4 text-rose-600 hover:text-rose-700 transition-all duration-200 rounded-2xl hover:bg-rose-50 dark:hover:bg-rose-900/20 font-semibold group"
                >
              ...
              <Link
                href="/login"
                className="flex items-center gap-4 px-4 py-4 text-white bg-gradient-to-r from-accent to-accent-dark transition-all duration-200 rounded-2xl hover:shadow-lg font-semibold group justify-center"
                onClick={() => setIsMobileMenuOpen(false)}
              >
                <LogIn className="w-5 h-5" />
                <span>Entrar</span>
